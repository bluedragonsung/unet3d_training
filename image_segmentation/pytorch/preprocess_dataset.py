...
class Preprocessor:
    ...
    def normalize_intensity(self, image: np.array):
        # 픽셀 값을 self.min_val과 self.max_val 사이로 제한
        image = np.clip(image, self.min_val, self.max_val)
    
        # MIN_CLIP_VAL에 해당하는 픽셀은 0으로 설정
        image_zeroed = np.where(image == self.min_val, 0, image)

        # 0으로 설정된 픽셀을 제외하고 정규화 수행
        image_normalized = np.where(image_zeroed != 0, 255*((image_zeroed - self.mean) / self.std - (self.min_val - self.mean) / self.std) / ((self.max_val - self.min_val) / self.std), 0)
