...
def main():
    mllog.config(filename=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'checkpoint_900.log'))
    mllog.config(filename=os.path.join("/DATA/soosung/unet3d/", 'checkpoint_900.log'))
    mllogger = mllog.get_mllogger()
    mllogger.logger.propagate = False
    mllog_start(key=constants.INIT_START)

    flags = PARSER.parse_args()
    print(flags.save_ckpt_path)
    print(sys.argv)
    ...
    # if flags.pretrained_weights is not None:
    #     checkpoint = torch.load(flags.pretrained_weights, map_location=device)
    #     model.load_state_dict(checkpoint['best_model_state_dict'])
    #     print(f"Loaded pretrained weights from {flags.pretrained_weights}")
