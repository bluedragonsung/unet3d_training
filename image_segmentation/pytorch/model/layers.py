...
layer_param_path = "/DATA/soosung/feature_map_numpy/layer"
layer_num = 0
...
normalizations = {
    "instancenorm": lambda n, _: nn.InstanceNorm3d(n, affine=True, track_running_stats=True),
...
class DownsampleBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        ############################### cha #####################################
        # np.save(layer_param_path + str(layer_num), x.detach().cpu().numpy())
        # layer_num += 1
        #########################################################################
        x = self.conv2(x)
        ############################### cha #####################################
